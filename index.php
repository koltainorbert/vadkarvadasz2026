...
(function(){
  ...
  function renderSpider(ctx,ctl,target){var el=target||spiderEl;if(!el)return;var sps=Object.keys(speciesMeta),n=sps.length,cx=180,cy=155,mR=95;function ang(i){return(2*Math.PI*i/n)-Math.PI/2;}function pt(i,v){var a=ang(i),r=mR*(v/100);return{x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)};}var g='';[20,40,60,80,100].forEach(function(lv,gi){var pts=sps.map(function(_,i){var p=pt(i,lv);return p.x+','+p.y;}).join(' ');g+='<polygon points="'+pts+'" fill="none" stroke="rgba(255,255,255,'+(gi===4?'.2':'.07')+')" stroke-width="1"/>';});var ax=sps.map(function(_,i){var p=pt(i,100);return '<line x1="'+cx+'" y1="'+cy+'" x2="'+p.x+'" y2="'+p.y+'" stroke="rgba(255,255,255,.14)" stroke-width="1"/>';}).join('');var scores=sps.map(function(sp){return scoreSpecies(sp,ctx,ctl).score;});var dp=scores.map(function(s,i){var p=pt(i,s);return p.x+','+p.y;}).join(' ');var poly='<polygon points="'+dp+'" fill="rgba(255,59,47,.22)" stroke="rgba(255,59,47,.85)" stroke-width="2.5" stroke-linejoin="round"/>';var dots=scores.map(function(s,i){var p=pt(i,s);return '<circle cx="'+p.x+'" cy="'+p.y+'" r="5" fill="#ff3b2f" data-sp="'+sps[i]+'" data-score="'+s+'" stroke="rgba(0,0,0,.4)" stroke-width="1.5" style="cursor:pointer"/>';}).join('');var labs=sps.map(function(sp,i){var a=ang(i),lp=pt(i,100),ox=Math.cos(a)*36,oy=Math.sin(a)*36,lx=lp.x+ox,ly=lp.y+oy,anc=lx<cx-8?'end':(lx>cx+8?'start':'middle'),s=scores[i];return '<text x="'+lx+'" y="'+(ly-4)+'" text-anchor="'+anc+'" font-size="8.5" font-weight="700" font-family="sans-serif" fill="rgba(255,255,255,.72)">'+speciesMeta[sp].label+'</text><text x="'+lx+'" y="'+(ly+12)+'" text-anchor="'+anc+'" font-size="14" font-weight="900" font-family="sans-serif" fill="#ff3b2f">'+s+'</text>';}).join('');el.innerHTML='<svg viewBox="0 0 380 350" xmlns="http://www.w3.org/2000/svg">'+g+ax+poly+dots+labs+'</svg>';el.querySelectorAll('circle[data-sp]').forEach(function(c){c.addEventListener('mouseenter',function(){c.setAttribute('r','8');c.style.filter='drop-shadow(0 0 5px #ff3b2f)';});c.addEventListener('mouseleave',function(){c.setAttribute('r','5');c.style.filter='';});c.addEventListener('click',function(){var all=el.querySelectorAll('circle[data-sp]');all.forEach(function(cc){cc.setAttribute('opacity','0.3');});c.setAttribute('opacity','1');setTimeout(function(){all.forEach(function(cc){cc.setAttribute('opacity','1');});},700);});});}
  function renderSpiderStats(forecast,top){if(!spiderStatsEl)return;var sps=Object.keys(speciesMeta),ctl=controls(),scores=sps.map(function(sp){return scoreSpecies(sp,top.ctx,ctl).score;});var b1='<div class="va-srs"><div class="va-srs__h">Fajok kockázata</div>'+sps.map(function(sp,i){var s=scores[i],lv=riskLevel(s);return '<div class="va-srs__row"><span class="va-srs__n">'+speciesMeta[sp].short+'</span><div class="va-srs__tr"><div class="va-srs__fi is-'+lv.cls+'" style="width:'+s+'%"></div></div><span class="va-srs__v">'+s+'</span></div>';}).join('')+'</div>';var days=forecast.slice(0,7),b2='<div class="va-srs"><div class="va-srs__h">7 napos trend</div><div class="va-srs__spark">'+days.map(function(day){var d=new Date(day.day+'T12:00:00'),lv=riskLevel(day.score),h=Math.max(4,Math.round(day.score*0.62));return '<div class="va-srs__col"><div class="va-srs__spb is-'+lv.cls+'" style="height:'+h+'px"></div><span class="va-srs__sd">'+dayName(d.getDay()).slice(0,1)+'</span></div>';}).join('')+'</div></div>';var s=top.score,lv=riskLevel(s),gcol=s>=85?'#ff0000':s>=70?'#ff3b2f':s>=50?'#ff6b00':s>=25?'#ff9800':'#4caf50';var b3='<div class="va-srs"><div class="va-srs__h">Napi csúcsk.',+... wait no, let me fix: b3 content directly without continuation`;var b3='<div class="va-srs"><div class="va-srs__h">Napi csúcskóckázat</div><svg viewBox="0 0 120 72" class="va-srs__gsv"><path d="M16,64 A44,44 0 0,0 104,64" fill="none" stroke="rgba(255,255,255,.1)" stroke-width="9" stroke-linecap="round"/><path d="M16,64 A44,44 0 0,0 104,64" fill="none" stroke="'+gcol+'" stroke-width="9" stroke-linecap="round" stroke-dasharray="'+s+' 100" pathLength="100"/><text x="60" y="55" text-anchor="middle" font-size="20" font-weight="900" font-family="sans-serif" fill="#fff">'+s+'</text><text x="60" y="67" text-anchor="middle" font-size="8" font-weight="700" font-family="sans-serif" fill="rgba(255,255,255,.55)">'+lv.label+'</text></svg></div>';spiderStatsEl.innerHTML=b1+b2+b3;}
