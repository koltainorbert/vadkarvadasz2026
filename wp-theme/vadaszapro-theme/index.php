...
(function initRadar(){
  ...
  var radarSvgTemplate=<?php
    $va_radar_svg_path = get_template_directory() . '/radar.svg';
    $va_radar_svg_raw = '';
    if ( file_exists( $va_radar_svg_path ) ) {
      $va_radar_svg_raw = (string) file_get_contents( $va_radar_svg_path );
    }
    echo wp_json_encode( $va_radar_svg_raw );
  ?>;
  ...
  function renderWidgetRadarCard(score){
    if(!spiderWidgetEl){return;}
    var safeScore=clamp(Math.round(score||0),0,100);
    if(!radarSvgTemplate){
      spiderWidgetEl.innerHTML='<div class="vhr-gauge__species">Radar SVG nem található</div>';
      return;
    }
    var svg=String(radarSvgTemplate);
    var minCenter=726.37;
    var maxCenter=6335.83;
    var logoWidth=500*1.53;
    var boardWidth=7000;
    var center=minCenter+((maxCenter-minCenter)*(safeScore/100));
    var tx=clamp(center-(logoWidth/2),0,boardWidth-logoWidth);
    svg=svg.replace(/transform="translate\(([-0-9.]+)\s+([-0-9.]+)\)\s+scale\(([-0-9.]+)\)"/,function(_m,_x,y,s){
      return 'transform="translate('+tx.toFixed(2)+' '+y+') scale('+s+')"';
    });
    svg=svg.replace('<svg ','<svg style="display:block;width:100%;height:auto;" ');
    spiderWidgetEl.innerHTML='<div class="vhr-logo-radar-wrap" style="width:100%;overflow:hidden;border-radius:14px;">'+svg+'</div>';
  }
