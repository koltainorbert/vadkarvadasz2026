...
.va-home-radar__score{font-size:2.7rem !important;line-height:.95 !important;}
...
(function initRadar(){
  ...
  function renderSpider(ctx,ctl,target){var el=target||spiderEl;if(!el)return;var isWidget=el===spiderWidgetEl;var sps=Object.keys(speciesMeta),n=sps.length,cx=170,cy=isWidget?145:175,mR=isWidget?72:75,labelOffset=isWidget?30:36,viewBox=isWidget?'0 12 380 250':'0 0 380 350';function ang(i){return(2*Math.PI*i/n)-Math.PI/2;}function pt(i,v){var a=ang(i),r=mR*(v/100);return{x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)};}var g='';[20,40,60,80,100].forEach(function(lv,gi){var pts=sps.map(function(_,i){var p=pt(i,lv);return p.x+','+p.y;}).join(' ');g+='<polygon points="'+pts+'" fill="none" stroke="rgba(255,255,255,'+(gi===4?'.18':'.06')+')" stroke-width="1"/>';});var ax=sps.map(function(_,i){var p=pt(i,100);return '<line x1="'+cx+'" y1="'+cy+'" x2="'+p.x+'" y2="'+p.y+'" stroke="rgba(255,255,255,.12)" stroke-width="1"/>';}).join('');var scores=sps.map(function(sp){return scoreSpecies(sp,ctx,ctl).score;});var dp=scores.map(function(s,i){var p=pt(i,s);return p.x+','+p.y;}).join(' ');var poly='<polygon points="'+dp+'" fill="rgba(255,59,47,.2)" stroke="rgba(255,59,47,.9)" stroke-width="2" stroke-linejoin="round"/>';var dots=scores.map(function(s,i){var p=pt(i,s);return '<circle cx="'+p.x+'" cy="'+p.y+'" r="5" fill="#ff3b2f" data-sp="'+sps[i]+'" data-score="'+s+'" stroke="rgba(0,0,0,.4)" stroke-width="1.5" style="cursor:pointer"/>';}).join('');var labs=sps.map(function(sp,i){var a=ang(i),lp=pt(i,100),ox=Math.cos(a)*labelOffset,oy=Math.sin(a)*labelOffset,lx=lp.x+ox,ly=lp.y+oy,anc=lx<cx-8?'end':(lx>cx+8?'start':'middle'),s=scores[i];return '<text x="'+lx+'" y="'+(ly-4)+'" text-anchor="'+anc+'" font-size="8.5" font-weight="700" font-family="sans-serif" fill="rgba(255,255,255,.72)">'+speciesMeta[sp].label+'</text><text x="'+lx+'" y="'+(ly+12)+'" text-anchor="'+anc+'" font-size="14" font-weight="900" font-family="sans-serif" fill="#ff3b2f">'+s+'</text>';}).join('');el.innerHTML='<svg viewBox="'+viewBox+'" xmlns="http://www.w3.org/2000/svg">'+g+ax+poly+dots+labs+'</svg>';el.querySelectorAll('circle[data-sp]').forEach(function(c){c.addEventListener('mouseenter',function(){c.setAttribute('r','8');c.style.filter='drop-shadow(0 0 5px #ff3b2f)';});c.addEventListener('mouseleave',function(){c.setAttribute('r','5');c.style.filter='';});c.addEventListener('click',function(){var all=el.querySelectorAll('circle[data-sp]');all.forEach(function(cc){cc.setAttribute('opacity','0.3');});c.setAttribute('opacity','1');setTimeout(function(){all.forEach(function(cc){cc.setAttribute('opacity','1');});},700);});});}
