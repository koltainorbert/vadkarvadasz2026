...
.va-home-radar__spider--widget{max-width:100%;margin:0 0 12px;}
...
(function(){
  ...
  function renderSpider(ctx,ctl,target){var el=target||spiderEl;if(!el)return;var sps=Object.keys(speciesMeta),n=sps.length,cx=160,cy=158,mR=108;function ang(i){return(2*Math.PI*i/n)-Math.PI/2;}function pt(i,v){var a=ang(i),r=mR*(v/100);return{x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)};}var g='';[20,40,60,80,100].forEach(function(lv,gi){var pts=sps.map(function(_,i){var p=pt(i,lv);return p.x+','+p.y;}).join(' ');g+='<polygon points="'+pts+'" fill="none" stroke="rgba(255,255,255,'+(gi===4?'.2':'.07')+')" stroke-width="1"/>';});var ax=sps.map(function(_,i){var p=pt(i,100);return '<line x1="'+cx+'" y1="'+cy+'" x2="'+p.x+'" y2="'+p.y+'" stroke="rgba(255,255,255,.14)" stroke-width="1"/>';}).join('');var scores=sps.map(function(sp){return scoreSpecies(sp,ctx,ctl).score;});var dp=scores.map(function(s,i){var p=pt(i,s);return p.x+','+p.y;}).join(' ');var poly='<polygon points="'+dp+'" fill="rgba(255,59,47,.22)" stroke="rgba(255,59,47,.85)" stroke-width="2.5" stroke-linejoin="round"/>';var dots=scores.map(function(s,i){var p=pt(i,s);return '<circle cx="'+p.x+'" cy="'+p.y+'" r="4.5" fill="#ff3b2f" stroke="rgba(0,0,0,.4)" stroke-width="1.5"/>';}).join('');var labs=sps.map(function(sp,i){var a=ang(i),lp=pt(i,100),ox=Math.cos(a)*20,oy=Math.sin(a)*20,lx=lp.x+ox,ly=lp.y+oy,anc=lx<cx-8?'end':(lx>cx+8?'start':'middle'),s=scores[i];return '<text x="'+lx+'" y="'+(ly-5)+'" text-anchor="'+anc+'" font-size="10" font-weight="700" font-family="sans-serif" fill="rgba(255,255,255,.75)">'+speciesMeta[sp].label+'</text><text x="'+lx+'" y="'+(ly+11)+'" text-anchor="'+anc+'" font-size="16" font-weight="900" font-family="sans-serif" fill="#ff3b2f">'+s+'</text>';}).join('');el.innerHTML='<svg viewBox="0 0 320 316" xmlns="http://www.w3.org/2000/svg">'+g+ax+poly+dots+labs+'</svg>';}
