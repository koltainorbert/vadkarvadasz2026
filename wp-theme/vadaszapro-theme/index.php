...
.va-home-radar__spider#vhrSpiderWidget{margin:-24px 0 -10px !important;max-width:none !important;overflow:hidden;border-color:rgba(120,190,255,.22);background:radial-gradient(circle at 50% 50%,rgba(65,155,255,.17),transparent 64%),linear-gradient(180deg,rgba(255,255,255,.045),rgba(255,255,255,.02));}
...
(function initRadar(){
  ...
  function renderSpider(ctx,ctl,target){var el=target||spiderEl;if(!el)return;var isWidget=el===spiderWidgetEl;var sps=Object.keys(speciesMeta),n=sps.length,cx=170,cy=isWidget?128:175,mR=isWidget?86:75,labelOffset=isWidget?20:36,viewBox=isWidget?'8 20 344 214':'0 0 380 350';function ang(i){return(2*Math.PI*i/n)-Math.PI/2;}function pt(i,v){var a=ang(i),r=mR*(v/100);return{x:cx+r*Math.cos(a),y:cy+r*Math.sin(a)};}var g='';[20,40,60,80,100].forEach(function(lv,gi){var pts=sps.map(function(_,i){var p=pt(i,lv);return p.x+','+p.y;}).join(' ');g+='<polygon points="'+pts+'" fill="none" stroke="rgba(255,255,255,'+(gi===4?'.18':'.06')+')" stroke-width="1"/>';});var ax=sps.map(function(_,i){var p=pt(i,100);return '<line x1="'+cx+'" y1="'+cy+'" x2="'+p.x+'" y2="'+p.y+'" stroke="rgba(255,255,255,.12)" stroke-width="1"/>';}).join('');var scores=sps.map(function(sp){return scoreSpecies(sp,ctx,ctl).score;});var dp=scores.map(function(s,i){var p=pt(i,s);return p.x+','+p.y;}).join(' ');var poly='<polygon points="'+dp+'" fill="rgba(58,150,255,.22)" stroke="rgba(95,180,255,.92)" stroke-width="2" stroke-linejoin="round"/>';var dots=scores.map(function(s,i){var p=pt(i,s);return '<circle cx="'+p.x+'" cy="'+p.y+'" r="5" fill="#63a9ff" data-sp="'+sps[i]+'" data-score="'+s+'" stroke="rgba(0,0,0,.4)" stroke-width="1.5" style="cursor:pointer"/>';}).join('');var labs=sps.map(function(sp,i){var a=ang(i),lp=pt(i,100),ox=Math.cos(a)*labelOffset,oy=Math.sin(a)*labelOffset,lx=lp.x+ox,ly=lp.y+oy,anc=lx<cx-8?'end':(lx>cx+8?'start':'middle'),s=scores[i];return '<text x="'+lx+'" y="'+(ly-4)+'" text-anchor="'+anc+'" font-size="8.5" font-weight="700" font-family="sans-serif" fill="rgba(255,255,255,.72)">'+speciesMeta[sp].label+'</text><text x="'+lx+'" y="'+(ly+12)+'" text-anchor="'+anc+'" font-size="14" font-weight="900" font-family="sans-serif" fill="#63a9ff">'+s+'</text>';}).join('');el.innerHTML='<svg viewBox="'+viewBox+'" xmlns="http://www.w3.org/2000/svg">'+g+ax+poly+dots+labs+'</svg>';el.querySelectorAll('circle[data-sp]').forEach(function(c){c.addEventListener('mouseenter',function(){c.setAttribute('r','8');c.style.filter='drop-shadow(0 0 5px #63a9ff)';});c.addEventListener('mouseleave',function(){c.setAttribute('r','5');c.style.filter='';});c.addEventListener('click',function(){var all=el.querySelectorAll('circle[data-sp]');all.forEach(function(cc){cc.setAttribute('opacity','0.3');});c.setAttribute('opacity','1');setTimeout(function(){all.forEach(function(cc){cc.setAttribute('opacity','1');});},700);});});}
...
.va-hn-tl{position:absolute;top:0;bottom:0;width:2px;background:rgba(255,0,0,.85);z-index:5;pointer-events:none;animation:va-hn-mid-pulse .9s ease-in-out infinite;}
